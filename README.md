

```zsh
╭─ ashly@github  ~/portfolio  main ✔
╰─❯ cat README.md
```

---

```zsh
╭─ ashly@github  ~/about  main ✔
╰─❯ whoami

  Ashly Genao Taveras
  Junior CS @ University of South Florida 🤘

╭─ ashly@github  ~/about  main ✔
╰─❯ cat bio.txt

  I am a Software Developer currently learning Full-Stack and DevOps
  and looking to contribute to real-world projects, grow my technical
  skills, and learn from experienced developers through internships.

╭─ ashly@github  ~/about  main ✔
╰─❯ cat status.txt

  Currently:
  Building personal projects
  Learning React 
```

---

```zsh
╭─ ashly@github  ~/skills  main ✔
╰─❯ ls -la

  drwxr-xr-x  React
  drwxr-xr-x  TypeScript
  drwxr-xr-x  JavaScript
  drwxr-xr-x  Express
  drwxr-xr-x  Python
  drwxr-xr-x  MongoDB
  drwxr-xr-x  Git
  drwxr-xr-x  HTML5 · CSS
  drwxr-xr-x  Tailwind CSS

```

---

```zsh
╭─ ashly@github  ~/projects  main ✔
╰─❯ git log --oneline --all
```

| Commit | Project | Stack | Description |
|--------|---------|-------|-------------|
| `a1b2c3d` | [Tracklytics](https://github.com/ashlygenaot/demo-marketing) | React . TailwindCSS . shadcn/ui | a responsive marketing dashboard featuring campaign CRUD, metrics, interactive charts, dark mode, and LocalStorage persistence.  |
| `e4f5g6h` | [Simple-Login](https://github.com/ashlygenaot/Simple-Login-) | React . TypeScript . HTML . TailwindCSS | A responsive college student profile app built with featuring authentication, form validation, editable user data, and a clean dashboard UI. |
| `h7z0q2l` | [Scale](https://github.com/ashlygenaot/scale)| React . JavaScript . TailwindCSS . Express . MongoDB . Recharts | A full-stack climbing logbook built to help climbers track sessions, projects, and long-term progress. |
| `b6l1p9x` | [CutBuddy](https://github.com/ashlygenaot/CutBuddy) | Python . AWS . Strands SDK . Claude Haiku . Jupyter Notebook . | A multi-agent AI system for personalized nutrition and meal planning. |
---

```zsh
╭─ ashly@github  ~/about  main ✔
╰─❯ cat fun_facts.py

  ashly = {
    "learning":    ["Full Stack Dev", "React", "DevOps", "Cloud", "Chrome Extensions"],
    "irl":         ["climbing 🧗‍♀️", "gym 🏋️‍♀️", "gaming 🎮", "drawing 🎨"]
    "career goal":  "Full-Stack Software Engineer or DevOps Engineer",
  }

╭─ ashly@github  ~/about  main ✔
╰─❯ █
```

---

```zsh
╭─ ashly@github  ~/connect  main ✔
╰─❯ open --links
```

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ashly-genao-taveras/)
```
